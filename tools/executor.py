...
class ToolExecutor:
    ...
    def _resolve_java_home(self) -> str:
        return _resolve_java_home()
    ...
    def _get_xml_property(self, filepath: str, name: str, default: str = "") -> str:
        try:
            for prop in ET.parse(filepath).getroot().findall("property"):
                n = prop.find("name")
                if n is not None and n.text == name:
                    v = prop.find("value")
    ...
    def _get_namenode_dir(self) -> str:
        """
        Return NameNode metadata dir as a plain filesystem path.
        Reads dfs.namenode.name.dir from hdfs-site.xml and strips file:// prefix.
        FIX: replaces the old hardcoded /tmp/hadoop-root/dfs/name/current check.
        """
        hdfs_site = os.path.join(self._hh(), "etc", "hadoop", "hdfs-site.xml")
        raw = self._get_xml_property(
            hdfs_site,
            "dfs.namenode.name.dir",
            f"file:///tmp/hadoop-{self._get_current_user()}/dfs/name",
        )
    ...
    def _install_hadoop(self, args: dict) -> dict:
        ...
        for url in mirrors:
            dl = self._run(["wget", "--continue", "--tries=3", "--timeout=60",
                            "--show-progress", "-O", tarfile, url], timeout=3600)
            if dl["returncode"] == 0:
                break
        if dl["returncode"] != 0:
    ...
    def _start_hdfs(self, args: dict) -> dict:
        ...
        if not ssh_ok:
            try:
                ...
                if not os.path.isfile(id_rsa):
                    subprocess.run(["ssh-keygen", "-t", "rsa", "-P", "", "-f", id_rsa],
        ...
        if not ssh_ok:
            return {
                ...
                "ssh_ready": False,
                "daemon_error": "SSH could not be configured.",
            }

        # FIX: read actual namenode dir from hdfs-site.xml instead of hardcoded path
        namenode_dir       = self._get_namenode_dir()
        namenode_formatted = os.path.isdir(os.path.join(namenode_dir, "current"))
        logger.info(f"NameNode dir={namenode_dir} formatted={namenode_formatted}")
        ...
        if not namenode_formatted:
            ...
            if fmt.returncode != 0:
                return {
                    ...
                    "stdout": fmt.stdout.strip(), "stderr": fmt.stderr.strip(),
                    "namenode_running": False, "datanode_running": False,
                    "secondary_namenode_running": False, "jps_output": "",
                    "ssh_ready": ssh_ok,
                    "daemon_error": f"NameNode format failed: {fmt.stderr.strip()}",
                }
